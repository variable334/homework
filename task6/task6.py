...
first = [round(randint(50,100)/10, 2) for _ in range(1,21)]
second = [round(randint(50,100)/10, 2) for _ in range(1,21)]

print("Первая команда:", first )
print("Первая команда:", second)

def WinnerIsPaired(list_1,list_2):
        res_list=[]
        for i in range(len(list_1)):
                if list_1[i] > list_2[i]:
                    res_list.append(list_1[i])
                else:
                    res_list.append(list_2[i])    
        return res_list 

a = WinnerIsPaired(first,second)
print(a)